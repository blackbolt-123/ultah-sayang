  ...
  window.Components.slideshow = {
    ...
    _showPhoto(div, index, animate = true, direction = "next") {
      ...
      if (animate) {
        ...
        gsap.set(div._caption, { opacity: 0 });
        ...
        const runAnim = () => {
          ...
          gsap.to(div._caption, {
            opacity: 1,
            duration: 0.4,
            delay: 0.2,
            ease: "power2.out",
          });
          // Heart float tetap ada tapi lebih subtle
          gsap.fromTo(div._heartTop,
            { opacity: 0.4, scale: 0.5, y: 10 },
            { opacity: 0, scale: 1, y: -20, duration: 1, ease: "power2.out" }
          );
        };
