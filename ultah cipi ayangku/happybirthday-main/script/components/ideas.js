  ...
  window.Components.ideas = {
    ...
    animate(tl, el) {
      ...
      regularLines.forEach((line) => {
        tl.fromTo(line, { ...ENTER }, { opacity: 1, y: 0, rotationX: 0, skewX: "0deg", duration: 0.8, ease: "power2.out" });

        const strong = line.querySelector("strong");
        if (strong) {
          tl.to(strong, {
            duration: 0.6, scale: 1.2, x: 10,
            backgroundColor: "var(--accent)", color: "#fff",
          });
        }

        // Increased display time from 2.5s to 3.5s for better readability
        tl.to(line, { duration: 0.8, ...LEAVE }, "+=3.5");
      });
      ...
      if (specialLine) {
        tl.fromTo(specialLine,
          ...
          { rotationX: 0, rotationZ: 0, skewY: "0deg", y: 0, z: 0, opacity: 1, duration: 0.8, ease: "power2.out" },
          "+=2"
        );

        const span = specialLine.querySelector("span");
        if (span) {
          tl.to(span, { duration: 0.8, rotation: 90, x: 8 }, "+=2");
        }

        // Increased display time for special line
        tl.to(specialLine, {
          duration: 0.8, scale: 0.2, opacity: 0,
        }, "+=3");
      }
      ...
      if (bigLetters.length) {
        tl.fromTo(bigLetters,
          ...
          { scale: 1, opacity: 1, rotation: 0, duration: 1, ease: "expo.out", stagger: 0.25 }
        )
        .to(bigLetters, {
          duration: 1, scale: 3, opacity: 0, rotation: -15,
          ease: "expo.out", stagger: 0.25,
        }, "+=2.5");
      }
    },
  };
